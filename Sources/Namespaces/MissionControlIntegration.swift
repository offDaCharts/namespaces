...
struct MissionControlLabelTarget: Equatable {
    enum Source: Equatable { case accessibility }
...
enum MissionControlLabelLocator {
    ...
    static func targets(model: AppModel) -> [MissionControlLabelTarget] {
        ...
        // v0.1.0's good spacing comes from these native AX anchors. Never
        // replace missing anchors with guessed equal slots: an ad-hoc update can
        // temporarily lose Accessibility approval, and guessed labels were the
        // unchanged bad row seen in 0.2.5–0.2.7.
        return resolved
    }
